"""Test: rhs_off with EI reactions enabled (proper k_ei_conc computation).

rhs_off가 이제 full RHS와 동일한 EI rate coefficient를 thermal Te로 계산.
3체 부착 포함 모든 EI 반응이 CX 기반으로 활성화.
"""
...
print(f"{'='*70}")
print(f"  rhs_off with EI enabled (CX-based k_ei_conc at thermal Te)")
print(f"  All reactions active: EI + TE_DEPENDENT + Arrhenius, P_dep=0")
print(f"  {N_TEST_PULSES} pulses")
print(f"{'='*70}")
...
for ip in range(N_TEST_PULSES):
    ...
    print(f"  Pulse {ip:3d}: ne={ne_peak:.3e} → {ne_valley:.3e} "
...
print(f"  Speed: {N_TEST_PULSES/wall:.1f} pulses/s")
